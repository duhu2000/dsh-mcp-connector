...
## [0.2.50-rc.1] - 2026-09-15

测试预发布，仅 npm `next`；不提升 `latest`。真实授权回调、进程清理、Web/Desktop 交互、查询和重启恢复尚未验收；Windows 自动授权不支持，企业管理员限制仍适用。

### Added

- P1 实验性浏览器授权入口：串行、取消、超时、登录后复核及敏感输出隔离；同源 UI 专用接口，不暴露为模型工具。默认关闭，需显式配置官方固定版本 CLI 安装目录；尚未完成真实登录验收。
