...
import { loadBundledCatalog, fetchRemoteCatalog, mergeCatalog, listCatalog, auditDescriptor } from './catalog.js';
  ...
  const api = {
    ...
    async importJson(json) {
      try {
        ...
        return { ok: true, message: `已导入并连接 ${keys.length} 个 MCP${extra}`, detail: { keys, skipped } };
      } catch (error) {
    ...
    async installFromUrl(url) {
      try {
        ...
        const normalized = list.map((raw) => auditDescriptor(normalizeConnectorDescriptor(raw)));
    ...
    async refreshCatalog() {
      if (!config.catalogUrl) {
        await recomputeMerged();
        return { ok: true, message: '无远程目录（catalogUrl 为空），已用内置目录刷新' };
      }
      try {
        ...
        await catalogStore.putRemote({ etag: fetched.etag, connectors: fetched.connectors }).catch(() => {});
        await recomputeMerged();
        return { ok: true, message: `已刷新目录，共 ${state.merged.length} 个连接器` };
      } catch (error) {
    ...
    async toolsList(connectorId) {
      ...
      if (!records.length) return { ok: false, message: `连接器 "${connector.name}" 尚未连接，请先连接` };
      ...
      for (const record of records) {
        ...
        // Add auth headers
        if (record.auth?.grantKey) {
          const grant = state.grants.get(record.auth.grantKey);
          if (grant?.grant?.accessToken) headers.Authorization = `Bearer ${grant.grant.accessToken}`;
        }
        ...
        try {
          ...
          if (!response.ok) {
            servers.push({ serverKey: record.serverKey || record.name, ok: false, error: `HTTP ${response.status}`, tools: [] });
            continue;
          }
          ...
          servers.push({ serverKey: record.serverKey || record.name, ok: false, error: error.message, tools: [] });
        }
      }
      ...
      return {
        ok: true,
        message: `共 ${servers.length} 个 server，${totalTools} 个工具`,
        detail: { connectorId, servers, totalTools },
      };
    },
  };
