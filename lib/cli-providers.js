...
/** P1 state contract only: no process launch, token storage or automatic login. */
export function transitionCliAuthorization(state, event) {
  const phases = ['idle', 'authorizing', 'checking', 'authorized', 'unauthorized', 'unknown', 'cancelled', 'timed-out', 'failed'];
  if (!state || !phases.includes(state.phase)) throw new Error('invalid authorization phase');
  if (!event || typeof event.type !== 'string') throw new Error('invalid authorization event');
  const active = ['authorizing', 'checking'].includes(state.phase);
  // Allowlisted output only. Never retain raw CLI output, tokens or identity.
  const next = (phase) => ({ phase, attemptId: state.attemptId ?? null });
  if (event.type === 'start') {
    if (active) throw new Error('authorization already in progress');
    if (!/^[A-Za-z0-9_-]{1,80}$/.test(event.attemptId ?? '') || event.attemptId === state.attemptId) {
      throw new Error('a new authorization attempt id is required');
    }
    return { phase: 'authorizing', attemptId: event.attemptId };
  }
  if (!active || event.attemptId !== state.attemptId) return next(state.phase);
  if (event.type === 'cancel') return next('cancelled');
  if (event.type === 'timeout') return next('timed-out');
  if (event.type === 'process-exit' && state.phase === 'authorizing') {
    // Login exit 0 still requires an independent official auth status check.
    return next(event.exitCode === 0 ? 'checking' : 'failed');
  }
  if (event.type === 'status' && state.phase === 'checking') {
    if (event.success !== true || typeof event.authenticated !== 'boolean') return next('unknown');
    return next(event.authenticated ? 'authorized' : 'unauthorized');
  }
  return next(state.phase);
}

/** Host-side lifecycle. Adapters must settle after abort and never log output. */
export function createCliAuthorizationManager({ launch, verify, timeoutMs = 180_000 } = {}) {
  if (typeof launch !== 'function' || typeof verify !== 'function') throw new Error('authorization adapters required');
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 300_000) throw new Error('invalid authorization timeout');
  let state = { phase: 'idle', attemptId: null };
  let active;
  let disposed = false;
  const snapshot = () => ({ ...state });
  const advance = (job, event) => {
    state = transitionCliAuthorization(state, { ...event, attemptId: job.id });
  };
  const stop = (type) => {
  ...
  return {
    status: snapshot,
    start() {
      if (disposed) throw new Error('authorization manager disposed');
      // Do not overlap official CLI writes, even while cancellation is settling.
      if (active) throw new Error('authorization already in progress or stopping');
      const job = { id: randomUUID(), controller: new AbortController() };
      state = transitionCliAuthorization(state, { type: 'start', attemptId: job.id });
      active = job;
      const timer = setTimeout(() => stop('timeout'), timeoutMs);
      timer.unref?.();
      job.done = Promise.resolve().then(async () => {
        if (job.controller.signal.aborted) return;
        const result = await launch({ signal: job.controller.signal });
        if (job.controller.signal.aborted) return;
        advance(job, { type: 'process-exit', exitCode: result?.exitCode });
        if (state.phase !== 'checking') return;
        const status = await verify({ signal: job.controller.signal });
        if (!job.controller.signal.aborted) advance(job, {
          type: 'status', success: status?.success, authenticated: status?.authenticated,
        });
      }).catch(() => {
        // Never serialize raw adapter errors: they may contain auth URLs/tokens.
        if (job.controller.signal.aborted) return;
        advance(job, state.phase === 'checking'
          ? { type: 'status', success: false }
          : { type: 'process-exit', exitCode: -1 });
      }).finally(() => {
        clearTimeout(timer);
    ...
    cancel(attemptId) {
      if (attemptId !== state.attemptId) return snapshot();
      return stop('cancel');
    },
    async settled() { await active?.done; return snapshot(); },
    async dispose() {
      disposed = true;
      stop('cancel');
      await active?.done;
