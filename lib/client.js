...
`;
		...
		function connectorSettingsCopy() {
			...
			if (String(language).toLowerCase().startsWith("en")) {
				return {
					...
					readOnly: "This plugin detail is read-only.",
					saving: "Saving…",
		...
		function ConnectorSettingsCard(props) {
			const { settingsScope, actions, view } = props;
			...
			/* The bundle-config slot is also queried for a one-line summary; the card belongs to the page view only. */
			if (view === "summary" || snapshot.status !== "ready") return null;
		...
		function installClientSettings(ctx, marketView) {
			...
			ctx.inject(["settingsScope"], (settingsCtx) => {
				...
				// dsh 0.1.7+ moved a plugin's own settings into its Plugins-page details card
				// and no longer declares the old Settings -> Plugins list slot. Register both:
				// a slot the running host does not declare simply never resolves, so one client
				// serves either host. The keyed slot matches on the BUNDLE package name.
		...
		function MarketOverlay(props) {
			...
			}, [actions, launchPromptSession, workspaceContext]);
			(0, react.useEffect)(() => {
								...
								children: [
									...
									style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", minWidth: 0, flex: 1 },
										children: [
											...
											versionCheckFailed && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: "mcpConnectorUpdateStatus",
												"data-tone": "error",
