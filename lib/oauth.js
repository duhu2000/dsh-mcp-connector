...
function serverMetadataUrls(issuer, suffix) {
  const url = new URL(issuer);
  if (url.search || url.hash || url.username || url.password) {
    throw new OAuthError('invalid_metadata', 'OAuth issuer must not contain query, fragment or credentials');
  }
  const path = url.pathname.replace(/\/+$/, '');
  // RFC 8414 inserts before the issuer path; retain suffix-form compatibility.
  return [...new Set([
    `${url.origin}/.well-known/${suffix}${path}`,
    `${issuer.replace(/\/+$/, '')}/.well-known/${suffix}`,
  ])];
}

function validateServerMetadata(body, issuer) {
  if (!body || typeof body !== 'object' || Array.isArray(body) || body.issuer !== issuer) {
    throw new OAuthError('invalid_metadata', 'OAuth metadata issuer must exactly match the requested issuer');
  }
  return body;
}

async function fetchServerMetadata(issuer, suffix, timeoutMs) {
  const urls = serverMetadataUrls(issuer, suffix);
  for (let index = 0; index < urls.length; index++) {
    try {
      return validateServerMetadata(await fetchJson(urls[index], { method: 'GET' }, timeoutMs), issuer);
    } catch (error) {
      // Do not mask issuer mismatches, access denial, rate limits or outages.
      if (index === urls.length - 1 || ![404, 405].includes(error.httpStatus)) throw error;
    }
  }
}
...
export async function discoverServerMetadata(issuer, timeoutMs) {
  ...
  const oauthBody = await fetchServerMetadata(issuer, 'oauth-authorization-server', timeoutMs);
  ...
  if (!oauthBody.registration_endpoint || !oauthBody.revocation_endpoint) {
    try {
      oidcBody = await fetchServerMetadata(issuer, 'openid-configuration', timeoutMs);
    } catch {
