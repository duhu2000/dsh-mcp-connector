...
export class McpHttpError extends Error {
  constructor(message, { kind = 'protocol', status, retryAfterMs } = {}) {
    super(message);
...
function responseError(response, payload, text) {
  ...
  if (response.status === 429) {
    return new McpHttpError('服务请求过于频繁（HTTP 429），请稍后重试', { kind: 'rate-limit', status: response.status, retryAfterMs: parseRetryAfter(response.headers.get('retry-after')) });
  }
