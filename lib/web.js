 ...
 *   - GET  /mcp-connector/events 无敏感数据的实时状态 SSE
 *
 ...
 * 返回 disposer（卸载三条 prefix 路由）。
 */
export function mountWebRoutes(wctx, api, { logger, eventHub, cliAuthService } = {}) {
  ...
  const apiDisposer = wctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const authMethod = new Map([['cliAuthStatus', 'status'], ['cliAuthStart', 'start'], ['cliAuthCancel', 'cancel']]).get(body?.method);
        if (authMethod) {
          // Auth operations require an explicit same-origin browser request.
          // They are never registered on the model-facing API/tool facade.
          const origin = header(req.headers, 'origin');
          const host = header(req.headers, 'host');
          let sameOrigin = false;
          try { const parsed = new URL(origin); sameOrigin = ['http:', 'https:'].includes(parsed.protocol) && parsed.host === host; } catch {}
          if (!sameOrigin) { writeJson(res, 403, { ok: false, message: 'forbidden' }); return; }
          if (!cliAuthService) { writeJson(res, 503, { ok: false, message: '页面授权不可用' }); return; }
          const params = body?.params ?? {};
          if (Object.keys(params).some((key) => !['confirmed', 'attemptId'].includes(key))) {
            writeJson(res, 400, { ok: false, message: '授权参数不受支持' }); return;
          }
          writeJson(res, 200, await cliAuthService[authMethod](params));
          return;
        }
  ...
  const eventsDisposer = wctx.webServer.register({
    kind: 'prefix',
    path: '/mcp-connector/events',
    handler: async (req, res) => {
      if (!fence(req)) {
        writeJson(res, 403, { ok: false, message: 'forbidden' });
        return;
      ...
      if (!eventHub) {
        writeJson(res, 503, { ok: false, message: 'status events unavailable' });
        return;
      }
      if (!eventHub.subscribe(req, res)) {
        writeJson(res, 503, { ok: false, message: 'too many status event clients' });
      }
    },
  });
