...
import { parseRetryAfter } from './discovery-policy.js';
...
export function classifyConnectionError(error, { transport } = {}) {
  ...
  if (text.includes('[CLI_AUTH_REQUIRED]')) {
    return { kind: 'authorization-required', message: '钉钉待授权：官方 CLI 当前未登录。若已确认企业尚未开放 CLI，请等待管理员处理，无需反复登录、重启或填写 API Key；具备授权条件后再完成官方登录并连接' };
  }
  if (text.includes('[CLI_AUTH_STATUS_INVALID]')) {
    return { kind: 'authorization-unknown', message: '钉钉授权状态未知：官方 CLI 返回异常或不完整响应，请检查 CLI 诊断；不能据此判断为未登录或管理员限制' };
  }
  ...
  if (/initial (?:connection|tool synchronization)|tool synchronization failed|initialize(?: handshake)? failed/i.test(text)) {
    return { kind: 'startup', message: transport === 'stdio'
      ? '本地 MCP 初始化或首次工具同步失败，Host 未提供可识别的具体原因；请检查官方 CLI 状态、启动命令和 Host 日志，不能仅据此判断为授权失败'
      : 'MCP 初始化或首次工具同步失败，请检查服务配置和 Host 日志' };
  }
  if (transport === 'stdio') {
...
export async function validateConnectionRecord(record, { timeoutMs = 15_000, fetchImpl = fetch, grants = new Map() } = {}) {
  ...
  try {
    ...
    if (response.status === 429) {
      return failed(record, 'rate-limit', '服务请求过于频繁（HTTP 429），请稍后重试', { httpStatus: response.status, retryAfterMs: parseRetryAfter(response.headers.get('retry-after')) });
    }
    if (!response.ok) {
