...
安装或升级后完全重启 DeepSeek Harness Desktop 或 `dsh web`，然后打开左侧「🧩 MCP连接器」；若侧边栏入口已隐藏，可在“插件”面板打开 `dsh-mcp-connector` 插件详情，点击“MCP连接器”卡片中的“打开 MCP连接器”。
...
- 侧边栏入口可按当前 profile 隐藏；隐藏后仍可从“插件 → dsh-mcp-connector → 插件详情”临时打开现有连接器弹框，用完即关。
...
1. 点击左侧“🧩 MCP连接器”，；若入口已隐藏，请在“插件”面板选择 `dsh-mcp-connector` 并点击其详情卡中的“打开 MCP连接器”。
...
当前公开版本为 [`dsh-mcp-connector@0.2.59`](https://www.npmjs.com/package/dsh-mcp-connector)，对应 [GitHub Release v0.2.59](https://github.com/duhu2000/dsh-mcp-connector/releases/tag/v0.2.59)。
