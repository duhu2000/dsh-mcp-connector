...
test('内置目录加载 + published 过滤', () => {
  ...
  const qccOAuthCards = qccCards.filter((d) => d.id !== 'qcc-document');
  assert.ok(qccOAuthCards.every((d) => d.auth.grantSharing === 'issuer'), '3 张企查查 OAuth 卡片应共享同 issuer OAuth Grant');
  ...
  assert.equal(qccDocument?.auth.mode, 'api-key');
  assert.equal(qccDocument?.auth.apiKeyHeader, 'Authorization');
  assert.match(qccDocument?.auth.credentialDescription, /不是企查查 OAuth 登录令牌/);
  ...
  assert.deepEqual(qccDocument?.servers[1]?.credentialBindings, { QCC_DOCUMENT_AUTHORIZATION: 'authorization' });
