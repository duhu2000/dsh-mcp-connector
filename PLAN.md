...
| 项目 | 状态 | 验收门禁 |
|---|---|---|
| 自动测试 | 已完成开发 | 75 项单元/集成测试 + 本地 UI mock 测试壳 |
| 连接健康状态 | 已完成开发 | 已配置/已连接/需重新授权/部分异常/连接异常/已停用；OAuth 与 Key 均支持 initialize 检查；工具发现对瞬时网络错误和 5xx 自动重试一次 |
| Desktop E2E | 0.2.11 实机回归已通过 | 完全重启、中文单语界面、9 张卡片、既有连接/授权无损；八爪鱼 OAuth 登录成功，真实枚举 1 个服务 / 12 个工具，`list_platforms` 只读调用成功 |
...
- 当前公开版本：`0.2.11`，用于八爪鱼标准 OAuth 2.1 + PKCE 动态注册兼容；75 项测试、npm Trusted Publishing 与 Desktop 真实授权回归均已通过。
- 许可证：MIT，公开仓库。
